...
class AIComparator
  ...
  def compare_ais
    ...
    match_ups.delete_if { |ai_1, ai_2| ai_1 == ai_2 }

    @games_to_play.times do |game_number|
      match_ups.each do |ai_1, ai_2|
        vs(ai_1, ai_2, game_number)
      end
    end
...
private
  ...
  def print_results
    ...
    table = Terminal::Table.new do |t|
      ...
      t.headings = ['AI Type', 'Placement', 'Wins', 'Loses']
      @competitors.each do |comp|
        # t << [comp.name, comp.placement_type, comp.rating, comp.wins, comp.loses]
        t << [comp.name, comp.placement_type, comp.wins, comp.loses]
      end
    end
    puts table
  end
